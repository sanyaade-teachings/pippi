...
int audio_callback(jack_nframes_t nframes, __attribute__((unused)) void * arg) {
    ...
    samples = nframes / ASTRID_CHANNELS;

    in1 = jack_port_get_buffer(astrid_inport1, samples);
    in2 = jack_port_get_buffer(astrid_inport2, samples);
    ...
    if(lpadc_increment_and_release(adcbuf, samples) < 0) {
